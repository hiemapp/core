...
export default class FlowBlockContext {
    ...
    protected callbacks: Array<(...args: any[]) => unknown> = [];
    protected nextCallbackId: number = 0;
    ...
    createCallback<T>(callback: (data: T, ...args: any[]) => unknown): (data: T) => unknown {
        const currentCallbackId = this.nextCallbackId++;
        this.callbacks[currentCallbackId] = callback;

        return (...args: any[]) => {
            if (typeof this.callbacks[currentCallbackId] !== 'function') return;
            return this.callbacks[currentCallbackId](...args);
        }
    }
    ...
    sibling(offset: number) {
        try {
            return this.siblings()[this.index() + offset];;
        } catch(err) {
            return null;
        }
    }
    ...
    toString(): string {
        return `[FlowBlock ${this.id} in ${this.flowCtx.flow}]`;
    }
